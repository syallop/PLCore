...
exprType exprBindCtx typeBindCtx typeCtx e = case e of
  ...
  Lam abs expr
    -> do let newExprBindCtx = addBinding (absTy abs) exprBindCtx
          exprTy <- exprType newExprBindCtx typeBindCtx typeCtx expr
  ...
  --       App f x : b
  App f x
    -> do fTy <- exprType exprBindCtx typeBindCtx typeCtx f -- Both f and x must type check
          xTy <- exprType exprBindCtx typeBindCtx typeCtx x
  ...
  Sum expr ix inTypr
    -> do -- Expression must type check
          exprTy <- exprType exprBindCtx typeBindCtx typeCtx expr
  ...
  Product prodExprs
    -> do -- type check each successive expression
          prodExprTys <- mapM (exprType exprBindCtx typeBindCtx typeCtx) prodExprs
  ...
  Union unionExpr unionTypeIndex unionTypes
    -> do -- type check injected expression
          exprTy <- exprType exprBindCtx typeBindCtx typeCtx unionExpr
  ...
  Case caseExpr (DefaultOnly defExpr)
    -> do -- caseExpr should be well typed (but we don't care about anything else)
          _ <- exprType exprBindCtx typeBindCtx typeCtx caseExpr

          -- The case expression is then typed by the default branch assuming its well typed
          exprType exprBindCtx typeBindCtx typeCtx defExpr
  ...
  Case caseExpr (CaseBranches (SomeCaseBranches branch0 branches) mDefExpr)
    -> do -- The expression case-analysed on must type-check
          caseExprTy <- exprType exprBindCtx typeBindCtx typeCtx caseExpr

          -- Check the first and any other branches
          branch0Ty <- branchType branch0 caseExprTy exprBindCtx typeBindCtx typeCtx
          branchTys <- mapM (\branch -> branchType branch caseExprTy exprBindCtx typeBindCtx typeCtx) branches

          -- Check the default branch if it exists
          mDefExprTy <- maybe (Right Nothing) (\defExpr -> Just <$> exprType exprBindCtx typeBindCtx typeCtx defExpr) mDefExpr
...
checkMatchWith :: (Binds b (Type tb),Ord tb) => MatchArg b tb -> Type tb -> ExprBindCtx b tb -> TypeBindCtx tb -> TypeCtx tb -> Either (Error tb) [Type tb]
checkMatchWith match expectTy exprBindCtx typeBindCtx typeCtx = do
  ...
  case match of
    ...
    MatchSum sumIndex nestedMatchArg
      -> do sumTypes <- case rExpectTy of
            ...
            checkMatchWith nestedMatchArg matchedTy exprBindCtx typeBindCtx typeCtx
    ...
    MatchProduct nestedMatchArgs
      -> do prodTypes <- case rExpectTy of
            ...
            checkMatchesWith nestedMatchArgs prodTypes exprBindCtx typeBindCtx typeCtx
    ...
    MatchUnion unionIndexTy nestedMatchArg
      -> do unionTypes <- case rExpectTy of
            ...
            checkMatchWith nestedMatchArg unionIndexTy exprBindCtx typeBindCtx typeCtx


checkMatchesWith :: (Binds b (Type tb),Ord tb) => [MatchArg b tb] -> [Type tb] -> ExprBindCtx b tb -> TypeBindCtx tb -> TypeCtx tb -> Either (Error tb) [Type tb]
checkMatchesWith matches types exprBindCtx typeBindCtx typeCtx = case (matches,types) of
  ([],[]) -> Right []
  ([],_)  -> Left $ EMsg "Expected more patterns in match"
  (_,[])  -> Left $ EMsg "Too many patterns in match"
  (m:ms,t:ts)
    -> checkMatchWith m t exprBindCtx typeBindCtx typeCtx >>= \boundTs -> checkMatchesWith ms ts exprBindCtx typeBindCtx typeCtx >>= Right . (boundTs ++)
