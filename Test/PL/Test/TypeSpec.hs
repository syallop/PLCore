...
spec = do
  describe "Types" $ do
    describe "Type (kind) check" $ typeChecksTypesSpec typeTestCases ppKind (ppError ppPattern ppType )
    describe "Reduce"            $ reducesTypesToSpec  typeTestCases ppType ppPattern
  where
